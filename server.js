...
try { const c = JSON.parse(fs.readFileSync(path.join(__dirname, 'config.json'), 'utf8')); Object.assign(config, c); } catch {}
  ...
  if (req.method === 'POST' && url.pathname === '/create-room') {
    ...
    req.on('end', () => {
      try {
        ...
        res.end(JSON.stringify({ roomId, name: room.name, motd: room.motd, maxPlayers: room.maxPlayers, url: '', worldSeed: config.worldSeed, pvp: config.pvp }));
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'admin_auth': {
        if (String(msg.password || '') === config.adminPassword && config.adminPassword) {
          p.isAdmin = true; try { ws.send(JSON.stringify({ type: 'admin_auth_result', success: true })); } catch {}
        } else { try { ws.send(JSON.stringify({ type: 'admin_auth_result', success: false, error: 'Wrong password' })); } catch {} }
        break;
      }
      case 'admin_command': {
...
httpServer.listen(PORT, () => {
  console.log(`\n============================================\n  VoxelHost Game Server\n============================================\n  Port: ${PORT}\n  Admin PW: ${config.adminPassword}\n============================================\n`);
});